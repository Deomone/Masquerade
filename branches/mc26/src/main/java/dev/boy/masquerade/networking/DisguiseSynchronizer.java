...
public final class DisguiseSynchronizer {
    ...
    public boolean canReveal(ServerPlayer viewer) {
    ...
    public void sendUpdate(ServerPlayer viewer, DisguiseState state) {
    ...
    public void sendSnapshot(ServerPlayer viewer, List<DisguiseState> states,
        ...
        if (ServerPlayNetworking.canSend(viewer, MasqueradePayloads.StateSnapshot.ID)) {
            ...
            ServerPlayNetworking.send(viewer, new MasqueradePayloads.StateSnapshot(unknownName, placeholder, states));
        }
    }
}
