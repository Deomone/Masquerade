...
public final class DeathHandler {
    ...
    private void dropStolenHead(ServerPlayerEntity victim, DamageSource source) {
        ...
        victim.dropStack(PlayerHeads.create(victim.getGameProfile()));
        MasqueradeLog.info("{} beheaded {}",
                context.identities().moderationName(killer), victim.getGameProfile().getName());
    }
    ...
    private void releaseDisguise(ServerPlayerEntity victim) {
        ...
        victim.dropStack(PlayerHeads.create(removed.get().toGameProfile()));
    }
}
